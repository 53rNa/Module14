...
# Создание Inline меню из 4 кнопок с надписями о продуктах
def create_product_inline_keyboard():
    keyboard = InlineKeyboardMarkup(row_width=4)
    for i in range(1, 5):
        keyboard.insert(InlineKeyboardButton(f"Product{i}", callback_data="product_buying"))
    return keyboard
...
async def get_buying_list(message: types.Message):
    for i in range(1, 5):
        ...
        await message.answer_photo(photo=open(f'{i}.jpg', 'rb'))
